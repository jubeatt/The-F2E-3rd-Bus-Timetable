    ...
    <div class="content">
      ...
      <div class="container">
        <template v-for="(stop, index) in stops" :key="index">
          <router-link
            :to="`/Search-Nearby-Stations/${$route.params.Station}/EstimatedTimeOfArrival/${currentCity}/${stop.RouteUID}`"
            @click="toNextPage"
            class="card-info">
...
export default {
  ...
  methods: {
    async getStation () {
      const data = await fetch(
        `https://ptx.transportdata.tw/MOTC/v2/Bus/Station/City/${this.currentCity}?$filter=StationName%2FZh_tw%20eq%20%27${this.$route.params.Station}%27&$format=JSON`,
    ...
    async getRouteInfo (routeUID) {
      const data = await fetch(
        `https://ptx.transportdata.tw/MOTC/v2/Bus/Route/City/Kaohsiung?$filter=RouteUID%20eq%20'${routeUID}'&$format=JSON`,
        { headers: GetAuthorizationHeader() }
      )
      const json = await data.json()
      return json
  ...
  async created () {
    ...
    this.station[0].Stops.forEach((item, index, originalArray) => {
      if (index !== 0) {
        if (
          originalArray[index].RouteName.Zh_tw !==
          originalArray[index - 1].RouteName.Zh_tw
        ) {
          // 暫存路線資料
